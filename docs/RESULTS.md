...
- `node-a`: primary server, bot, and agent.
- `node-c`: secondary monitored VPS.

All labelled fault-injection experiments (n = 30) were run on `node-b`,
decommissioned after the evaluation; the data is retained as ground truth.

As of the latest verification:

- Fleet status: 2 total, 2 online, 0 degraded, 0 offline.
- Server API, Telegram bot, and agents active on both nodes.
- Incident API enabled and returning active incident lifecycle state.

## Labelled Fault-Injection Experiments (n = 30)

Two injection types on `node-b`:
