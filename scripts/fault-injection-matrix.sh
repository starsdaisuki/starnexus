...
#
# Defaults: 3 reps × [30s, 60s, 150s, 300s] on node-b. ~70 minutes wall time.
set -euo pipefail
...
Options:
  --ssh-host <alias>       SSH alias of the experimental node (default: node-b).
  --node-id <id>           StarNexus node id (default: node-b).
  --server-ssh <alias>     Primary server SSH alias (default: node-a).
