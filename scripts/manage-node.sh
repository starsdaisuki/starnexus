...
#
# Prerequisites:
#   - SSH config aliases for your VPS (e.g. "node-a", "node-c")
...
load_or_detect_primary() {
  ...
  ask_required "Primary server SSH alias (e.g. node-a)" PRIMARY_SSH
...
cmd_add() {
  ...
  ask_required "Node ID (lowercase, no spaces, e.g. node-c)" NODE_ID
  ask "Display name (e.g. Node C)" NODE_NAME "$NODE_ID"
