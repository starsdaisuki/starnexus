...
func TestHTTPMiddlewareRecordsRequests(t *testing.T) {
	...
	for _, path := range []string{"/api/nodes", "/api/nodes/node-a/details", "/api/nodes/node-b/details"} {
...
func TestNormalizePath(t *testing.T) {
	cases := []struct {
		...
		{"/", "/"},
		{"/api/nodes", "/api/nodes"},
		{"/api/nodes/node-a", "/api/nodes/{id}"},
		{"/api/nodes/node-a/details", "/api/nodes/{id}/details"},
