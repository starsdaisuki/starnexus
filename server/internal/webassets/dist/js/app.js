const StarApp = (() => {
  ...
  function bindUI() {
    ...
    const basemapSelect = document.getElementById('map-basemap')
    if (basemapSelect) {
      StarMap.listBasemaps().forEach(({ key, label }) => {
        const option = document.createElement('option')
        option.value = key
        option.textContent = label
        basemapSelect.appendChild(option)
      })
      basemapSelect.value = StarMap.getBasemap()
      basemapSelect.addEventListener('change', event => {
        StarMap.setBasemap(event.target.value)
      })
    }
  ...
  async function fetchBenchmark() {
    ...
    // live Go server. Deployments without the artifact keep the panel
    // hidden instead of showing a wall of placeholders.
    try {
      const response = await fetch('data/benchmark.json', { cache: 'no-cache' })
      if (!response.ok) throw new Error(`bench ${response.status}`)
      const data = await response.json()
      document.getElementById('panel-benchmark').classList.remove('hidden')
      renderBenchmark(data)
    } catch (error) {
      document.getElementById('panel-benchmark').classList.add('hidden')
    }
  }
  ...
  async function fetchScalability() {
    ...
    try {
      ...
      const data = await response.json()
      document.getElementById('panel-scalability').classList.remove('hidden')
      renderScalability(data)
    } catch (error) {
      document.getElementById('panel-scalability').classList.add('hidden')
    }
  }
  ...
  function renderDashboard() {
    ...
    const emptyHint = document.getElementById('map-empty-hint')
    if (emptyHint) emptyHint.classList.toggle('hidden', nodes.length > 0)
  ...
  function renderGroundTruth(groundTruth) {
    ...
    const panel = document.getElementById('panel-experiments')
    root.innerHTML = ''

    // Only research deployments have labelled fault-injection
    // experiments — hide the panel entirely elsewhere.
