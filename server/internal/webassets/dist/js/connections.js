const StarConns = (() => {
  ...
  function buildTooltip(conn, node) {
    return `
      <div>
        <strong>${escapeHtml(conn.src_ip)}</strong> → <strong>${escapeHtml(node.name)}</strong><br>
        <span>${escapeHtml(conn.src_city || '?')}${conn.src_country ? `, ${escapeHtml(conn.src_country)}` : ''}</span><br>
        <span>${escapeHtml(conn.protocol || 'unknown')} • ${formatRate(conn.rate || 0)} • ${formatBytes(conn.total_bytes || 0)}</span>
      </div>
    `
  }
